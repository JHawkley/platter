...
quadNone = QuadTree.FLAGS.none;
quadTL = QuadTree.FLAGS.topLt
quadTR = QuadTree.FLAGS.topRt
quadBL = QuadTree.FLAGS.botLt
quadBR = QuadTree.FLAGS.botRt
...
describe 'platter: broad-phase, quad-tree', ->
  ...
  it 'should obtain appropriate quadrants for a variety of objects', ->
    expect(tree._getQuads(new Box(2.5, 2.5, 5, 5))).toBe quadTL
    expect(tree._getQuads(new Box(12.5, 2.5, 5, 5))).toBe quadTR
    expect(tree._getQuads(new Box(2.5, 12.5, 5, 5))).toBe quadBL
    expect(tree._getQuads(new Box(12.5, 12.5, 5, 5))).toBe quadBR
    expect(tree._getQuads(new Box(7.5, 7.5, 5, 5))).toBe quadAll
    expect(tree._getQuads(new Box(2.5, -10, 5, 5))).toBe quadTL
    expect(tree._getQuads(new Box(21, 10, 5, 5))).toBe (quadTR | quadBR)
    expect(tree._getQuads(new Box(2.5, 7.5, 5, 5))).toBe (quadTL | quadBL)
  
  it 'should split when (maxObjects + 1) objects are inserted', ->
    spyOn(tree, '_split').and.callThrough()
    tree.insert new Box(0, 0, 4, 4)
    tree.insert new Box(2, 3, 5, 7)
    tree.insert new Box(9, 11, 6, 4)
    tree.insert new Box(4, 4, 6, 6)
    expect(tree._split).not.toHaveBeenCalled()
    tree.insert new Box(15, 15, 4, 4)
    expect(tree._split).toHaveBeenCalled()
  ...
  it 'should count touching as being in a quadrant', ->
    ...
    expect(tree.objects).toEqual similarArray([boxes[2], boxes[3], boxes[4], boxes[5], boxes[6]])
    expect(tree.nodes[quadTL].objects).toEqual [boxes[0]]
