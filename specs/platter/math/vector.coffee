...
describe 'platter: math, mutable vector', ->
  ...
  describe 'operators', ->
    ...
    it 'should be able to test equality', ->
      vec2 = MutableVector.create(4, 4)

      expect(vec1.equalTo(vec2)).toBe true

      vec2 = MutableVector.create(1, 3)

      expect(vec1.equalTo(vec2)).toBe false

      expect(vec1.equalTo(null)).toBe false
...
describe 'platter: math, immutable vector', ->
  ...
  it 'should not mutate `x` or `y` by setting other properties', ->
    vector = new ImmutableVector(30, 20)
  ...
  describe 'operators', ->
    ...
    it 'should be able to test equality', ->
      vec2 = ImmutableVector.create(4, 4)

      expect(vec1.equalTo(vec2)).toBe true

      vec2 = ImmutableVector.create(1, 3)
