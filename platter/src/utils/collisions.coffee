...
# Since this system only works on two axii, `x` and `y`, any other
# separating axis will need to be projected back onto these to properly
# separate the two objects.
xAxis = Vector.create(1, 0)
yAxis = Vector.create(0, 1)

# Working penetration, for intersection tests.
wp = { x: 0, y: 0 }

aabb_aabb_penetration = (a, b, out) ->
  x = Math.max(a.x, b.x)
  num1 = Math.min(a.x + a.width, b.x + b.width)
  y = Math.max(a.y, b.y)
  num2 = Math.min(a.y + a.height, b.y + b.height)
  out.x = num1 - x; out.y = num2
  return out

aabb_aabb_intersection = (a, b) ->
  aabb_aabb_penetration(a, b, wp)
  return wp.x >= 0 and wp.y >= 0
